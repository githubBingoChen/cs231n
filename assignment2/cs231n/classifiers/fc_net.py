...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        relu, relu_cache = relu_forward(fc1)
        fc2, fc2_cache = affine_forward(relu, W2, b2)
        scores = fc2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        # use the backward function defined before 
        reg = self.reg
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):  
            ...
            if i == self.num_layers - 1:
                ...
                if self.use_dropout:
                    dropout_out, dp_cache = dropout_forward(relu_out, self.dropout_param)
                    layer_input = dropout_out
                else:
                    layer_input = relu_out
                
            layer_caches.append((fc_cache, relu_cache, bn_cache, dp_cache))
        ...
        for i in reversed(range(self.num_layers)): 
            fc_cache, relu_cache, bn_cache, dp_cache = layer_caches[i]
            if i == self.num_layers - 1:
